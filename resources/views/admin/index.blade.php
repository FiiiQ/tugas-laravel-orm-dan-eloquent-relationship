        ...
        <div class="row bg-info rounded px-3 py-3 w-100">
            <div class="d-flex justify-content-between">
                ...
                <div class="col-md-4 d-flex justify-content-end">
                    <a href="{{ route('users.index', ['user' => $user->id]) }}" class="btn btn-md btn-primary fw-semibold my-auto me-1">Lihat Profil</a>
                    <a href="{{ route('products.create', ['user' => $user->id]) }}" class="btn btn-md btn-dark fw-semibold my-auto me-1">Tambah
                        Produk</a>
                    <a href="{{ route('products.index') }}" class="btn btn-md btn-secondary fw-semibold my-auto">Kembali ke
                        Produk</a>
                </div>
            </div>
                        ...
                        <tr>
                            ...
                            @if ($product->kondisi == 'Baru')
                                <td class="text-center">
                                    <div class="rounded px-3 py-1 bg-success w-50 mx-auto fw-semibold">{{ $product->kondisi }}
                                    </div>
                                </td>
                            @else
                                <td class="text-center">
                            ...
                            @endif
                            <td class="d-flex">
                                <a href="{{ route('products.edit', ['product' => $product->id, 'user' => $user->id]) }}"
                                    class="btn btn-warning btn-md">Update</a>
                                <form
                                    action="{{ route('products.delete', ['product' => $product->id, 'user' => $user->id]) }}"
